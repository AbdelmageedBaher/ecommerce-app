  ...
  return (
    ...
    <div className='pt-15 flex items-center container'>
        <div className='w-1/3 pt-16'>
           <img src={imgSrc?imgSrc:dataObj?.imageCover} className='w-full transition-all' alt="" />
           <div className='flex flex-wrap'>
            {dataObj?.images?.map((image,index)=> <img src={image} index={index} onClick={changeImage} className={`w-[20%] mx-1 cursor-pointer object-cover ${index==ind?' border-2 border-main-color opacity-100':'opacity-50'}`}  key={image} />)}
           </div>
        </div>
        ...
        <div className='flex justify-between py-3'>
            ...
            <span>{dataObj?.ratingsAverage} <i class="fa-solid fa-star text-rating-color"></i> </span>
        </div>
        <div onClick={()=>{mutate(dataObj?._id)}} className="btn bg-main-color py-2 d-block mx-auto rounded-lg text-white text-center cursor-pointer">
            add to cart
        </div>
        </div>
    </div>
...
<div className='container pt-10'>
    <h2 className='text-[2rem] font-bold text-main-color py-4'>Related Product:</h2>
      <div className="flex flex-wrap">
        {Related.length?Related.map((prod)=> <ProductItem key={prod._id} prod={prod}></ProductItem>):<Loading/>}
      </div>
    </div>
</>
  )
